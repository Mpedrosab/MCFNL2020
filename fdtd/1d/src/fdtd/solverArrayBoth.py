...
class Solver:
    ...
    def _updateE(self, t, dt):
        ...
        if self._dispLayer is not None:
            ...
            cE2 = (2*dt/((2*self._epsilon*sp.epsilon_0+np.sum(2*np.real(self._bp),1))*self._mesh.steps()))
        ...
        for lu in range(2):
            ...
            if self._mesh.bounds[lu] == "pec":
                ...
                if self._dispLayer is not None:
                    eNewDisp[pos] = eDisp[pos] + 2*cE2*(hDisp[pos] if pos == 0 else -hDisp[pos])
                ...
                if pos == 0:
                    ...
                    if self._dispLayer is not None:
                        eNewDisp[-1] = eDisp[-2]+(sp.speed_of_light*dt-self._mesh.steps())* (eNewDisp[-2]-eDisp[ 1]) / (sp.speed_of_light*dt+self._mesh.steps())
...
#Get also field for free particle
        if self._dispLayer is not None:
            ...
            cH2 = dt / sp.mu_0 / self._mesh.steps()
